...
$("#render").addEventListener("click", async () => {
  ...
  const chunk = Number.isNaN(parseInt($("#chunk").value))
    ? longer > 72
      ? Math.ceil(width / 4)
      : 1
    : Math.round(parseInt($("#chunk").value));

  if (chunk == 0)
    $(".render-overlay > #prog").innerText = "Progress unknown (chunk = 0)";

  window.pixels = pixels;
  for (var i = 0; i < pixels.length; i++) {
    const item = pixels[i];
    $(".render-overlay > #prog").innerText = `Pixel #${i} of ${
      pixels.length
    } (${Math.round((i / pixels.length) * 100)}% done)`;

    if (chunk !== 0 && i % chunk === 0) await sleep(0.1);
  }
